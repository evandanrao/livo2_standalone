...
void loadVoxelConfig(const params::Params &p, VoxelMapConfig &voxel_config) {
  // Stage 4: reads from params struct (populated by params::load() from
  // livo2.yaml)
  // Wire publish-plane-map flag from the runtime params.
  voxel_config.is_pub_plane_map_ = p.slam.pub_plane_map_en;
  voxel_config.max_layer_ = p.slam.max_layer;
  voxel_config.max_voxel_size_ = p.slam.voxel_size;
  voxel_config.planner_threshold_ = p.slam.min_eigen_value;
  voxel_config.sigma_num_ = 3;
  voxel_config.beam_err_ = p.slam.beam_err;
  voxel_config.dept_err_ = p.slam.dept_err;
  voxel_config.layer_init_num_ = p.slam.layer_init_num;
  voxel_config.max_points_num_ = p.slam.max_points_num;
  voxel_config.max_iterations_ = p.slam.max_iterations;
  voxel_config.map_sliding_en = p.slam.map_sliding_en;
  voxel_config.half_map_size = p.slam.half_map_size;
  voxel_config.sliding_thresh = p.slam.sliding_thresh;
}
...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat) {
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_;
    ...
    // euler_cur removed: geoQuat_ is now set from rot_end directly
    ...
    if (!EKF_stop_flg && (rematch_num >= 2 ||
      ...
      geoQuat_ = Eigen::Quaterniond(
          state_.rot_end); // Stage 3: replaced
...
PointCloudXYZI::Ptr VoxelMapManager::pubVoxelMap() {
  // Throttle to ~2 Hz (assuming ~10 Hz call rate from the mapping thread).
  // Matches the behaviour of the ROS1 version which skips 4 of every 5 frames.
  static int frame_count = 0;
  if (++frame_count % 5 != 0) {
    return nullptr;
  }

  // Walk the entire voxel map and collect planes that were updated this cycle.
  std::vector<VoxelPlane> plane_list;
  plane_list.reserve(1000);
  for (auto &kv : voxel_map_) {
    GetUpdatePlane(kv.second, config_setting_.max_layer_, plane_list);
  }
  if (plane_list.empty()) {
    return nullptr;
  }

  // Pack each confirmed plane's centroid into an XYZI point cloud.
  // intensity = 1.0 means "is_plane = true" (non-plane voxels are skipped).

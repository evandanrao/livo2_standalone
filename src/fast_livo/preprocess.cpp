...
Preprocess::Preprocess()
    : feature_enabled(0), lidar_type(HESAI_JT128), blind(0.01),
      point_filter_num(1) {
  inf_bound = 10;
  N_SCANS = 6;
  group_size = 8;
...
void Preprocess::process(const PointCloudXYZI::Ptr &cloud_in,
                         PointCloudXYZI::Ptr &pcl_out) {
  // Stage 3.3: cloud_in is already filtered and has curvature=per-point time
  // offset (ms). lidar_driver.cpp does the JT128-specific handling (ring
  // filter, blind, curvature encoding). Here we just apply point_filter_num
  // decimation and pass through.
  pl_surf.clear();
  if (!cloud_in || cloud_in->empty())
    return;
  pl_surf.reserve(cloud_in->size());
  for (size_t i = 0; i < cloud_in->size(); i++) {
    if (i % point_filter_num != 0)
      continue;
    pl_surf.points.push_back(cloud_in->points[i]);
  }
  *pcl_out = pl_surf;
}

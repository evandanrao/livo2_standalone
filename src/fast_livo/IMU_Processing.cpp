...
ImuProcess::ImuProcess()
  ...
  last_imu = livo::ImuData{}; // Stage 3.1
...
void ImuProcess::Reset() {
  fprintf(stderr, "[IMU_Processing] Reset\n");
  mean_acc = V3D(0, 0, -1.0);
  mean_gyr = V3D(0, 0, 0);
  angvel_last = Zero3d;
  imu_need_init = true;
...
void ImuProcess::IMU_init(const MeasureGroup &meas, StatesGroup &state_inout,
  ...
  fprintf(stderr, "[IMU_Processing] Initializing: %.1f %%\n",
  ...
  if (b_first_frame) {
    ...
    N = 1;
    b_first_frame = false;
    mean_acc = meas.imu.front().accel; // Stage 3.1
    mean_gyr = meas.imu.front().gyro;
  }

  for (const auto &imu : meas.imu) {
    cur_acc = imu.accel; // Stage 3.1
    cur_gyr = imu.gyro;
...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas,
  ...
  const double &imu_beg_time = v_imu.front().timestamp; // Stage 3.1
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++) {
      ...
      acc_avr << 0.5 * (head.accel.x() + tail.accel.x()),
          0.5 * (head.accel.y() + tail.accel.y()),
          0.5 * (head.accel.z() + tail.accel.z());

      fout_imu << setw(10) << head.timestamp - first_lidar_time << " "
      ...
      if (head.timestamp < prop_beg_time) {
        dt = tail.timestamp - last_prop_end_time;
...
void ImuProcess::Process2(LidarMeasureGroup &lidar_meas, StatesGroup &stat,
  ...
  if (imu_need_init) {
    ...
    if (init_iter_num > MAX_INI_COUNT) {
      ...
      imu_need_init = false;
      fprintf(stderr,
              "[IMU_Processing] IMU Initials: Gravity: %.4f %.4f %.4f %.4f; "
              "acc covarience: "
              ...
              cov_gyr[1], cov_gyr[2]);
      fprintf(stderr,
              "[IMU_Processing] IMU Initials: ba covarience: %.8f %.8f %.8f; "
              "bg covarience: "
              "%.8f %.8f %.8f\n",

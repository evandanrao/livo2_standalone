...
static constexpr int     FRAME_OVERHEAD = 11;       // 2 SOP + 2 pktid + 1 cmd + 2 ver + 2 len + 2 CRC
...
// Broadcast target — same subnet as lio_lc
static constexpr const char    *BROADCAST_ADDR = "192.168.168.255";
static constexpr uint16_t       BROADCAST_PORT = 8892;
...
// CMD_LIDAR_PATH — chunk header (matches LidarPathHeader in lidar_message_parser.cpp)
struct PathHeader {
    uint16_t path_seq;
    uint8_t  chunk_id;
    uint8_t  total_chunks;
...
// ── Publisher thread ──────────────────────────────────────────────────────────
void lidar_publisher_thread(livo::Bridge &bridge, const params::Params &) {
    ...
    struct PoseEntry { float x, y, z, qx, qy, qz, qw; };
    ...
    auto send_path = [&]() {
        ...
        for (int c = 0; c < nchunks && idx < total; c++) {
            ...
            int plen = build_packet(packet_buf, pkt_id, CMD_LIDAR_PATH,
                                    chunk, payload_len);
            sendto(sock, packet_buf, plen, 0,
    ...
    while (bridge.running.load()) {
        ...
        // ── Drain pose_queue — feed odom + path accumulator ────────────────
        {
            ...
            while (!local.empty()) {
                ...
                PoseEntry e{
                    (float)pd.position.x(), (float)pd.position.y(), (float)pd.position.z(),
                ...
                if (odom_st.due(now)) {
                    ...
                    pl.x = e.x; pl.y = e.y; pl.z = e.z;
                    pl.qx = e.qx; pl.qy = e.qy; pl.qz = e.qz; pl.qw = e.qw;
                    pl.keyframe_count = 0;
                    pl.status         = 1; // mapping
        ...
        if (scan_st.due(now)) {
            ...
            if (has_cloud && cd.cloud && !cd.cloud->empty()) {
                ...
                int total_chunks= (total_pts + MAX_POINTS_PER_CHUNK - 1) / MAX_POINTS_PER_CHUNK;
                ...
                for (int c = 0; c < total_chunks && pt_idx < total_pts; c++) {
                    ...
                    uint8_t chunk[sizeof(ScanHeader) + MAX_POINTS_PER_CHUNK * sizeof(ScanPoint)];
                    ...
                    for (int p = 0; p < n; p++, pt_idx++) {
                        ...
                        pts[p].intensity = (uint8_t)std::min(255.0f, std::max(0.0f, pt.intensity));
                        pts[p].reserved  = 0;
                    }

                    int payload_len = (int)(sizeof(ScanHeader) + n * sizeof(ScanPoint));
                    int plen = build_packet(packet_buf, pkt_id, CMD_LIDAR_SCAN,
                                            chunk, payload_len);
                    sendto(sock, packet_buf, plen, 0,
                           (struct sockaddr *)&dest, sizeof(dest));
                }

...
//   8. AcquisitionStart → warm-up stop/revoke/re-announce/restart → skip 10
//   frames
...
//
// VimbaX SDK expected at /opt/VimbaX_2025-3 (override via cmake
// -DVIMBAX_ROOT=).
...
static void bayer_rg8_to_rgb(const uint8_t *bayer, uint8_t *rgb, int width,
                             int height) {
...
static VmbError_t set_enum(VmbHandle_t h, const char *f, const char *v) {
  return VmbFeatureEnumSet(h, f, v);
}
static VmbError_t set_int(VmbHandle_t h, const char *f, VmbInt64_t v) {
  return VmbFeatureIntSet(h, f, v);
}
static VmbError_t set_float(VmbHandle_t h, const char *f, double v) {
  return VmbFeatureFloatSet(h, f, v);
}
static VmbError_t set_bool(VmbHandle_t h, const char *f, VmbBool_t v) {
  return VmbFeatureBoolSet(h, f, v);
}
...
// ── Clean close ──────────────────────────────────────────────────────────────
static void revoke_and_close(VmbHandle_t handle,
                             std::vector<VmbFrame_t> &frames) {
...
void camera_driver_thread(livo::Bridge &bridge, const params::Params &p) {
  ...
  fprintf(stderr, "[camera_driver] payload cam starting (VimbaX)  roi=%dx%d\n",
          DESIRED_WIDTH, DESIRED_HEIGHT);

  // ── Step 1: VmbStartup ────────────────────────────────────────────────────
  if (VmbStartup(nullptr) != VmbErrorSuccess) {
    fprintf(stderr,
            "[camera_driver] VmbStartup failed — camera thread exiting\n");
    return;
  }
  ...
  while (bridge.running.load() && sdk_ok) {
    ...
    if (num_found == 0) {
      ...
      if (std::chrono::steady_clock::now() >= detection_deadline) {
        fprintf(stderr, "[camera_driver] %ds timeout — restarting VimbaX SDK\n",
                TIMEOUT_SEC);
        VmbShutdown();
        std::this_thread::sleep_for(std::chrono::seconds(1));
        if (VmbStartup(nullptr) != VmbErrorSuccess) {
          fprintf(stderr,
                  "[camera_driver] VmbStartup re-init failed — exiting\n");
          sdk_ok = false;
          break;
        }
    ...
    VmbError_t open_err =
        VmbCameraOpen(cam.cameraIdString, VmbAccessModeFull, &handle);
    if (open_err != VmbErrorSuccess) {
      fprintf(stderr, "[camera_driver] FULL open failed (%d), trying READ\n",
              open_err);
    ...
    if (access_used == VmbAccessModeFull) {
      ...
      if (sw > 0 && sh > 0) {
        ...
        fprintf(stderr,
                ...
                (long)sw, (long)sh, DESIRED_WIDTH, DESIRED_HEIGHT, (long)ox,
                (long)oy);
      }
    ...
    const int act_h =
        static_cast<int>(get_int(handle, "Height", DESIRED_HEIGHT));
    fprintf(stderr, "[camera_driver] configured %dx%d @ 30 fps BayerRG8\n",
            act_w, act_h);
    ...
    std::vector<std::vector<uint8_t>> buffers(
        NUM_FRAMES, std::vector<uint8_t>(payload_size, 0));
    ...
    while (bridge.running.load()) {
      VmbError_t result =
          VmbCaptureFrameWait(handle, &frames[frame_index], 50 /*ms*/);
      ...
      if (result != VmbErrorSuccess) {
        ...
        if (result == -19 /*disconnect*/) {
          fprintf(stderr, "[camera_driver] camera disconnected (err -19) "
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now -
                                                             last_frame_time)
                .count() > 5) {
          fprintf(stderr, "[camera_driver] no frames for 5 s "
                          "— restarting detection\n");
          revoke_and_close(handle, frames);
          break;
        }
      ...
      // Validate: skip if first 32 bytes are zero or >50 % of first 1024 are
      // zero
      const uint8_t *raw = static_cast<const uint8_t *>(fr.buffer);
      const std::size_t check =
          std::min<std::size_t>(1024, static_cast<std::size_t>(act_w) * act_h);

...
/* main planning API */
bool PolyTrajOptimizer::optimizeTrajectory(const Eigen::MatrixXd &iniState,
  ...
  do {
    ...
    int result = lbfgs::lbfgs_optimize(
        variable_num_, x_init, &final_cost,
        PolyTrajOptimizer::costFunctionCallback, NULL,
        PolyTrajOptimizer::earlyExitCallback, this, &lbfgs_params);

    t2 = ego_now_s();
    double time_ms = (t2 - t1) * 1000;
    double total_time_ms = (t2 - t0) * 1000;
    ...
    if (result == lbfgs::LBFGS_CONVERGENCE ||
      ...
      for (size_t i = 0; i < swarm_trajs_->size(); ++i) {
        flag_swarm_too_close |=
            min_ellip_dist2_[i] <
            pow((swarm_clearance_ + swarm_trajs_->at(i).des_clearance) * 1.25,
                2);
      }
      if (!flag_swarm_too_close) {
        if (finelyCheckAndSetConstraintPoints(segments_nouse, jerkOpt_,
                                              false) == CHK_RET::OBS_FREE) {

          flag_success = true;
          PRINTF_COND("\033[32miter=%d,time(ms)=%5.3f,total_t(ms)=%5.3f,cost=%"
                      "5.3f\n\033[0m",
          ...
          restart_nums++;
          PRINTF_COND("\033[32miter=%d,time(ms)=%5.3f, fine check collided, "
                      "keep optimizing\n\033[0m",
                      iter_num_, time_ms);
        }
      } else {
        PRINTF_COND("Swarm clearance not satisfied, keep optimizing. "
                    "iter=%d,time(ms)=%5.3f, wei_swarm_mod_=%f\n",
      ...
      fprintf(stderr,
              "[WARN] "
              "Solver error. Return = %d, %s. Skip this planning.",
...
bool PolyTrajOptimizer::computePointsToCheck(poly_traj::Trajectory &traj,
                                             int id_cps_end,
                                             PtsChk_t &pts_check) {
  pts_check.clear();
  ...
  while (true) {
      ...
      if (touch_goal_ && pts_check.size() > 0) {
        ...
        if (pts_check.size() <= 0) {
          fprintf(
              stderr,
              "[ERROR] "
              "Failed to get points list to check (0x02). pts_check.size()=%d",
              (int)pts_check.size());
          return false;
        } else {
          return true;
        ...
        fprintf(stderr,
                "[ERROR] "
                "Failed to get points list to check (0x01). touch_goal_=%d, "
                "pts_check.size()=%d",
    ...
    if (t < 1e-5 || pts_check[id_cps_curr].size() == 0 ||
        (pt - pt_last).cwiseAbs().maxCoeff() > RES_2) {
      pts_check[id_cps_curr].emplace_back(
          std::pair<double, Eigen::Vector3d>(t, pt));
      pt_last = pt;
    }
...
PolyTrajOptimizer::CHK_RET PolyTrajOptimizer::finelyCheckAndSetConstraintPoints(
  ...
  int i_end = ConstraintPoints::two_thirds_id(
      init_points, touch_goal_); // only check closed 2/3 points.
    ...
    for (size_t j = 0; j < pts_check[i].size(); ++j) {
      ...
      if (flag_got_start && flag_got_end) {
        ...
        if (in_id < 0 || out_id < 0) {
          fprintf(stderr,
                  "[ERROR] "
                  "Should not happen! in_id=%d, out_id=%d",
                  in_id, out_id);
          return CHK_RET::ERR;
        }
  ...
  for (size_t i = 0; i < segment_ids.size(); ++i) {
    ...
    if (ret == ASTAR_RET::SUCCESS) {
      a_star_pathes.push_back(a_star_->getPath());
               ...
               i + 1 < segment_ids.size()) // connect the next segment
    {
      segment_ids[i].second = segment_ids[i + 1].second;
      segment_ids.erase(segment_ids.begin() + i + 1);
      --i;
      fprintf(stderr, "[WARN] "
                      "A corner case 2, I have never exeam it.\n");
    } else {
      fprintf(stderr, "[WARN] "
                      "A-star error, force return!\n");
      return CHK_RET::ERR;
    }
  }
  ...
  constexpr double MINIMUM_PERCENT =
      0.0; // Each segment is guaranteed to have sufficient points to generate
           // sufficient force
  ...
  for (size_t i = 0; i < segment_ids.size(); i++) {
    ...
    for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j) {
      ...
      int Astar_id = a_star_pathes[i].size() / 2,
          last_Astar_id; // Let "Astar_id = id_of_the_most_far_away_Astar_point"
                         // will be better, but it needs more computation
      ...
      while (true) {
        ...
        if (val * init_val <= 0 &&
            (abs(val) > 0 ||
             abs(init_val) > 0)) // val = init_val = 0.0 is not allowed
        {
          intersection_point =
              ...
              ((a_star_pathes[i][Astar_id] - a_star_pathes[i][last_Astar_id]) *
               (ctrl_pts_law.dot(init_points.col(j) -
                ...
                ctrl_pts_law.dot(a_star_pathes[i][Astar_id] -
                                 a_star_pathes[i][last_Astar_id])) // = t
              );
    ...
    /* Corner case: the segment length is too short. Here the control points may
     * outside the A* path, leading to opposite gradient direction. So I have to
    ...
    if (got_intersection_id >= 0) {
      ...
      // fprintf(stderr, "[ERROR] " "Failed to generate direction!
      // segment_id=%d", i);
    }
  }
...
bool PolyTrajOptimizer::roughlyCheckConstraintPoints(void) {
  ...
  int i_end = ConstraintPoints::two_thirds_id(
      cps_.points, touch_goal_); // only check closed 2/3 points.
  for (int i = 1; i <= i_end; ++i) {
    ...
    if (occ) {
      for (size_t k = 0; k < cps_.direction[i].size(); ++k) {
        if ((cps_.points.col(i) - cps_.base_point[i][k])
                .dot(cps_.direction[i][k]) <
            1 * grid_map_->getResolution()) // current point is outside all the
                                            // collision_points.
        {
          occ = false;
          break;
    ...
    if (occ) {
      ...
      if (j < 0) // fail to get the obs free point
      {
        fprintf(stderr,
                "[ERROR] "
                "The drone is in obstacle. It means a crash in real-world.\n");
        in_id = 0;
      }
      ...
      {
        fprintf(stderr, "[WARN] "
                        "Local target in collision, skip this planning.\n");
  ...
  if (flag_new_obs_valid) {
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i) {
      ...
      ASTAR_RET ret = a_star_->AstarSearch(
          /*(in-out).norm()/10+0.05*/ grid_map_->getResolution(), in, out);
      if (ret == ASTAR_RET::SUCCESS) {
        a_star_pathes.push_back(a_star_->getPath());
      } else if (ret == ASTAR_RET::SEARCH_ERR &&
                 i + 1 < segment_ids.size()) // connect the next segment
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i) {
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j) {
        Eigen::Vector3d ctrl_pts_law(cps_.points.col(j + 1) -
            ...
            intersection_point;
        int Astar_id = a_star_pathes[i].size() / 2,
            last_Astar_id; // Let "Astar_id =
                           // id_of_the_most_far_away_Astar_point" will be
                           // better, but it needs more computation
    ...
    if (!multitopology_data_.initial_obstacles_avoided) {
      ...
      for (int i = 1; i < cps_.points.cols() - 1; ++i) {
        if (cps_.base_point[i].size() > 0) {
          // Only adopts "0" since finelyCheckAndSetConstraintPoints() after one
          // optimization can add more base_points.
...
PolyTrajOptimizer::distinctiveTrajs(vector<std::pair<int, int>> segments) {
  ...
  for (int i = 0; i < seg_upbound; i++) {
    ...
    exit_multi_loop2:;
      ...
      if (occ_start_id == -1 || occ_end_id == -1) {
        // It means that the first or the last control points of one segment are
        // in obstacles, which is not allowed. fprintf(stderr, "[WARN] " "What?
        // occ_start_id=%d, occ_end_id=%d", occ_start_id, occ_end_id);
      ...
      // 1.2 Reverse the vector and find new base points from occ_start_id to
      // occ_end_id.
      for (int j = occ_start_id; j <= occ_end_id; j++) {
        ...
        if (RichInfoSegs[i].first.base_point[j].size() != 1) {
          cout << "RichInfoSegs[" << i << "].first.base_point[" << j
               << "].size()=" << RichInfoSegs[i].first.base_point[j].size()
               << endl;
          fprintf(stderr,
                  "[ERROR] "
                  "Wrong number of base_points!!! Should not be happen!.\n");

          cout << setprecision(5);
          cout << "cps_" << endl;
          cout << " clearance=" << obs_clearance_
               << " cps.size=" << cps_.cp_size << endl;
          for (int temp_i = 0; temp_i < cps_.cp_size; temp_i++) {
            if (cps_.base_point[temp_i].size() > 1 &&
                cps_.base_point[temp_i].size() < 1000) {
              fprintf(stderr, "[ERROR] "
                              "Should not happen!!!\n");
              cout << "######" << cps_.points.col(temp_i).transpose() << endl;
              for (size_t temp_j = 0; temp_j < cps_.base_point[temp_i].size();
                   temp_j++)
                cout << "      " << cps_.base_point[temp_i][temp_j].transpose()
                     << " @ " << cps_.direction[temp_i][temp_j].transpose()
                     << endl;
            }
          }
        ...
        if (grid_map_->getInflateOccupancy(base_pt_reverse)) // Search outward.
        {
          ...
          if (l > l_upbound) {
            fprintf(stderr,
                    "[WARN] "
                    "Can't find the new base points at the opposite within the "
                    "threshold. i=%d, j=%d",
                    i, j);
            ...
            goto exit_multi_loop3; // break "for (int j = 0; j <
                                   // RichInfoSegs[i].first.size; j++)"
          }
        } else if ((base_pt_reverse - RichInfoSegs[i].first.points.col(j))
                       .norm() >= RESOLUTION) // Unnecessary to search.
        {
          ...
          if (VERBOSE_OUTPUT)
            cout << "base_point="
                 << RichInfoSegs[i].first.base_point[j][0].transpose()
                 << " control point="
                 << RichInfoSegs[i].first.points.col(j).transpose() << endl;
    ...
    exit_multi_loop3:;
                     ...
                     .norm() >= RESOLUTION) // Unnecessary to search.
      {
        ...
        if (VERBOSE_OUTPUT)
          cout << "base_point="
               << RichInfoSegs[i].first.base_point[0][0].transpose()
               << " control point="
  ...
  if (seg_upbound == 0) // After the erase operation above, segment legth will
                        // decrease to 0 again.
  {
    std::vector<ConstraintPoints> oneSeg;
    oneSeg.push_back(cps_);
  ...
  for (int i = 0; i < max_traj_nums; i++) {
    ...
    while (digit_id < seg_upbound && selection[digit_id] >= VARIS) {
      ...
      if (digit_id >= seg_upbound) {
        fprintf(stderr,
                "[ERROR] "
                "Should not happen!!! digit_id=%d, seg_upbound=%d",
    ...
    while (/*seg_id < RichInfoSegs.size() ||*/ cp_id < cps_.cp_size) {
      ...
      if (seg_id >= seg_upbound || cp_id < segments[seg_id].first ||
        ...
        fprintf(stderr,
                "[ERROR] Should not happen! cp_id=%d, seg_id=%d, front=%d, "
                "back=%d, seg_first=%d, seg_second=%d\n",
                cp_id, seg_id, segments.front().first, segments.back().second,
                segments[seg_id].first, segments[seg_id].second);
      }
...
double PolyTrajOptimizer::costFunctionCallback(void *func_data, const double *x,
                                               double *grad, const int n) {
  PolyTrajOptimizer *opt = reinterpret_cast<PolyTrajOptimizer *>(func_data);

  fill(opt->min_ellip_dist2_.begin(), opt->min_ellip_dist2_.end(),
       std::numeric_limits<double>::max());

  Eigen::Map<const Eigen::MatrixXd> P(x, 3, opt->piece_num_ - 1);
  // Eigen::VectorXd T(Eigen::VectorXd::Constant(piece_nums, opt->t2T(x[n -
  // 1]))); // same t
  Eigen::Map<const Eigen::VectorXd> t(x + (3 * (opt->piece_num_ - 1)),
                                      opt->piece_num_);
  Eigen::Map<Eigen::MatrixXd> gradP(grad, 3, opt->piece_num_ - 1);
  Eigen::Map<Eigen::VectorXd> gradt(grad + (3 * (opt->piece_num_ - 1)),
                                    opt->piece_num_);
  ...
  opt->addPVAJGradCost2CT(gradT, obs_swarm_feas_qvar_costs,
                          opt->cps_num_prePiece_); // Time int cost
  ...
  opt->VirtualTGradCost(T, t, gradT, gradt,
                        time_cost); // Real time back to virtual time
...
template <typename EIGENVEC>
void PolyTrajOptimizer::addPVAJGradCost2CT(EIGENVEC &gdT,
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (int j = 0; j <= K; ++j) {
      ...
      if (swarmGradCostP(i_dp, t + step * j, pos, vel, gradp, gradt,
                         grad_prev_t, costp)) {
        gradViolaPc = beta0 * gradp.transpose();
...
bool PolyTrajOptimizer::obstacleGradCostP(const int i_dp,
                                          ...
                                          double &costp) {
  if (i_dp == 0 ||
      i_dp > ConstraintPoints::two_thirds_id(
                 cps_.points, touch_goal_)) // only apply to first 2/3
    return false;
...
bool PolyTrajOptimizer::swarmGradCostP(const int i_dp, const double t,
  ...
  for (size_t id = 0; id < swarm_trajs_->size(); id++) {
    if ((swarm_trajs_->at(id).drone_id < 0) ||
        swarm_trajs_->at(id).drone_id == drone_id_) {
      continue;
    }

    double traj_i_satrt_time = swarm_trajs_->at(id).start_time;
    double pt_time =
        (t_now_ - traj_i_satrt_time) +
        t; // never assign a high-precision golbal time to a double directly!
    const double CLEARANCE =
        (swarm_clearance_ + swarm_trajs_->at(id).des_clearance) *
    ...
    if (pt_time < swarm_trajs_->at(id).duration) {
      ...
      swarm_p =
          swarm_trajs_->at(id).traj.getPos(swarm_trajs_->at(id).duration) +
          exceed_time * swarm_v;
    }
    Eigen::Vector3d dist_vec = p - swarm_p;
    double ellip_dist2 =
        dist_vec(2) * dist_vec(2) * inv_a2 +
...
/* helper functions */
void PolyTrajOptimizer::setParam(const OptimizerParams &p) {
  cps_num_prePiece_ = p.cps_num_prePiece;
  wei_obs_ = p.wei_obs;
  wei_obs_soft_ = p.wei_obs_soft;
  wei_swarm_ = p.wei_swarm;
  wei_feas_ = p.wei_feas;
  wei_sqrvar_ = p.wei_sqrvar;
  wei_time_ = p.wei_time;
  obs_clearance_ = p.obs_clearance;
  obs_clearance_soft_ = p.obs_clearance_soft;
  swarm_clearance_ = p.swarm_clearance;
  max_vel_ = p.max_vel;
  max_acc_ = p.max_acc;
  max_jer_ = p.max_jer;
}
...
void PolyTrajOptimizer::setUseMultitopologyTrajs(bool use_multitopology_trajs) {

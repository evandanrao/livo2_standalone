...
// Wall-clock seconds using steady_clock
...
bool AStar::ConvertToIndexAndAdjustStartEndPoints(Vector3d start_pt,
  ...
  if (checkOccupancy(Index2Coord(start_idx))) {
    do {
      ...
      if (occ == -1) {
        fprintf(stderr, "[Astar] Start point outside the map region.\n");
        return false;
      }
    } while (occ);
  }
  ...
  if (checkOccupancy(Index2Coord(end_idx))) {
    do {
      ...
      occ = checkOccupancy(Index2Coord(start_idx));
      if (occ == -1) {
        fprintf(stderr, "[Astar] End point outside the map region.\n");
        return false;
      }
...
ASTAR_RET AStar::AstarSearch(const double step_size, Vector3d start_pt,
  ...
  if (!ConvertToIndexAndAdjustStartEndPoints(start_pt, end_pt, start_idx,
    ...
    fprintf(
        stderr,
        "[Astar] Unable to handle the initial or end point, force return!\n");
  ...
  while (!openSet_.empty()) {
    ...
    double time_2 = ego_now_s();
    if ((time_2 - time_1) > 0.2) {
      fprintf(stderr, "[Astar] Search timeout (0.2 s limit).\n");
  ...
  if ((time_2 - time_1) > 0.1)
    fprintf(stderr, "[Astar] Path finding took %.3fs, iter=%d\n",
            time_2 - time_1, num_iter);

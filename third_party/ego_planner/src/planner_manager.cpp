...
void EGOPlannerManager::initPlanModules(const PlannerParams &p,
                                        PlanningVisualization::Ptr vis) {
  /* read algorithm parameters */
  pp_.max_vel_ = p.max_vel;
  pp_.max_acc_ = p.max_acc;
  pp_.feasibility_tolerance_ = p.feasibility_tolerance;
  pp_.polyTraj_piece_length = p.polyTraj_piece_length;
  pp_.planning_horizen_ = p.planning_horizon;
  pp_.use_multitopology_trajs = p.use_multitopology_trajs;
  pp_.drone_id = p.drone_id;

  grid_map_.reset(new GridMap);
  grid_map_->initMap(p.grid_map);

  ploy_traj_opt_.reset(new PolyTrajOptimizer);
  ploy_traj_opt_->setParam(p.optimizer);
...
bool EGOPlannerManager::reboundReplan(
  ...
  double t_start = ego_now_s();
  double t_init, t_opt;

  static int count = 0;
  cout << "\033[47;30m\n[" << t_start << "] Drone " << pp_.drone_id
       << " Replan " << count++ << "\033[0m" << endl;
  // cout.precision(3);
  ...
  Eigen::MatrixXd cstr_pts =
      initMJO.getInitConstraintPoints(ploy_traj_opt_->get_cps_num_prePiece_());
  vector<std::pair<int, int>> segments;
  if (ploy_traj_opt_->finelyCheckAndSetConstraintPoints(
          segments, initMJO, true) == PolyTrajOptimizer::CHK_RET::ERR) {
    return false;
  }

  t_init = ego_now_s() - t_start;
  ...
  // fprintf(stderr, "[ERROR] " "BBBB" "\n");

  if (pp_.use_multitopology_trajs) {
    std::vector<ConstraintPoints> trajs =
        ploy_traj_opt_->distinctiveTrajs(segments);
    ...
    headState << initTraj.getJuncPos(0), initTraj.getJuncVel(0),
        initTraj.getJuncAcc(0);
    tailState << initTraj.getJuncPos(PN), initTraj.getJuncVel(PN),
        initTraj.getJuncAcc(PN);
    double final_cost, min_cost = 999999.0;

    for (int i = trajs.size() - 1; i >= 0; i--) {
      ploy_traj_opt_->setConstraintPoints(trajs[i]);
      ploy_traj_opt_->setUseMultitopologyTrajs(true);
      if (ploy_traj_opt_->optimizeTrajectory(headState, tailState, innerPts,
                                             initTraj.getDurations(),
                                             final_cost)) {
        success[i] = true;
        ...
        Eigen::MatrixXd ctrl_pts_temp =
            ploy_traj_opt_->getMinJerkOpt().getInitConstraintPoints(
    ...
    t_opt = ego_now_s() - t_start;

    if (trajs.size() > 1) {
      cout << "\033[1;33m"
           << "multi-trajs=" << trajs.size() << ",\033[1;0m"
           << " Success:fail=" << success.sum() << ":"
           << success.size() - success.sum() << endl;
    }

    visualization_->displayMultiOptimalPathList(
        vis_trajs,
    ...
    tailState << initTraj.getJuncPos(PN), initTraj.getJuncVel(PN),
        initTraj.getJuncAcc(PN);
    double final_cost;
    flag_success = ploy_traj_opt_->optimizeTrajectory(
  ...
  if (flag_success) {
    ...
    sum_time += (t_init + t_opt);
    count_success++;
    printf("Time:\033[42m%.3fms,\033[0m init:%.3fms, optimize:%.3fms, "
           "avg=%.3fms\n",
           (t_init + t_opt) * 1000, t_init * 1000, t_opt * 1000,
           sum_time / count_success * 1000);
    // cout << "total time:\033[42m" << (t_init + t_opt)
    //      << "\033[0m,init:" << t_init
    //      << ",optimize:" << t_opt
    //      << ",avg_time=" << sum_time / count_success << endl;

    setLocalTrajFromOpt(best_MJO, touch_goal);
    cstr_pts = best_MJO.getInitConstraintPoints(
        ploy_traj_opt_->get_cps_num_prePiece_());
    visualization_->displayOptimalList(cstr_pts, 0);

    continous_failures_count_ = 0;
  } else {
    cstr_pts = ploy_traj_opt_->getMinJerkOpt().getInitConstraintPoints(
        ploy_traj_opt_->get_cps_num_prePiece_());
...
bool EGOPlannerManager::computeInitState(
  ...
  if (flag_first_call ||
      flag_polyInit) /*** case 1: polynomial initialization ***/
  {
    ...
    if (!flag_randomPolyTraj) {
      if (innerPs.cols() != 0) {
        fprintf(stderr, "[ERROR] innerPs.cols() != 0 \n");
      }
      ...
      piece_dur_vec =
          Eigen::Vector2d(init_of_init_totaldur / 2, init_of_init_totaldur / 2);
    }
    ...
    piece_nums = round((headState.col(0) - tailState.col(0)).norm() /
    ...
    if (id != piece_nums - 1) {
      fprintf(stderr, "[ERROR] Should not happen! x_x\n");
      return false;
    }
    initMJO.reset(headState, tailState, piece_nums);
    initMJO.generate(innerPs, piece_dur_vec);
  } else /*** case 2: initialize from previous optimal trajectory ***/
  {
    if (traj_.global_traj.last_glb_t_of_lc_tgt < 0.0) {
      fprintf(stderr,
              "[ERROR] You are initialzing a trajectory from a previous "
              "optimal trajectory, but no previous trajectories up to now. \n");
      return false;
    }

    /* the trajectory time system is a little bit complicated... */
    double passed_t_on_lctraj = ego_now_s() - traj_.local_traj.start_time;
    double t_to_lc_end = traj_.local_traj.duration - passed_t_on_lctraj;
    if (t_to_lc_end < 0) {
      fprintf(stderr, "[INFO] "
                      "t_to_lc_end < 0, exit and wait for another call.\n");
      return false;
    }
    double t_to_lc_tgt = t_to_lc_end + (traj_.global_traj.glb_t_of_lc_tgt -
                                        traj_.global_traj.last_glb_t_of_lc_tgt);
    int piece_nums =
        ceil((start_pt - local_target_pt).norm() / pp_.polyTraj_piece_length);
    if (piece_nums < 2)
      piece_nums = 2;
    ...
    Eigen::VectorXd piece_dur_vec =
        Eigen::VectorXd::Constant(piece_nums, t_to_lc_tgt / piece_nums);
    headState << start_pt, start_vel, start_acc;
    tailState << local_target_pt, local_target_vel, Eigen::Vector3d::Zero();
    ...
    for (int i = 0; i < piece_nums - 1; ++i) {
      if (t < t_to_lc_end) {
        ...
        fprintf(stderr,
                "[ERROR] "
                "Should not happen! x_x 0x88 t=%.2f, t_to_lc_end=%.2f, "
                "t_to_lc_tgt=%.2f\n",
                t, t_to_lc_end, t_to_lc_tgt);
      }
...
void EGOPlannerManager::getLocalTarget(const double planning_horizen,
  ...
  for (t = traj_.global_traj.glb_t_of_lc_tgt;
    ...
    Eigen::Vector3d pos_t =
        traj_.global_traj.traj.getPos(t - traj_.global_traj.global_start_time);
  ...
  if ((t - traj_.global_traj.global_start_time) >=
      traj_.global_traj.duration - 1e-5) // Last global point
  {
    local_target_pos = global_end_pt;
    traj_.global_traj.glb_t_of_lc_tgt =
        traj_.global_traj.global_start_time + traj_.global_traj.duration;
    touch_goal = true;
  }

  if ((global_end_pt - local_target_pos).norm() <
      (pp_.max_vel_ * pp_.max_vel_) / (2 * pp_.max_acc_)) {
    local_target_vel = Eigen::Vector3d::Zero();
  } else {
    local_target_vel =
        traj_.global_traj.traj.getVel(t - traj_.global_traj.global_start_time);
  }
}

bool EGOPlannerManager::setLocalTrajFromOpt(const poly_traj::MinJerkOpt &opt,
  ...
  bool ret = ploy_traj_opt_->computePointsToCheck(
      traj, ConstraintPoints::two_thirds_id(cps, touch_goal), pts_to_check);
  if (ret && pts_to_check.size() >= 1 && pts_to_check.back().size() >= 1) {
    traj_.setLocalTraj(traj, pts_to_check, ego_now_s());
  }
...
bool EGOPlannerManager::checkCollision(int drone_id) {
  if (traj_.local_traj.start_time <
      1e9) // It means my first planning has not started
    return false;
  if (traj_.swarm_traj[drone_id].drone_id !=
      drone_id) // The trajectory is invalid
    return false;
  ...
  double t_end =
      min(my_traj_start_time + traj_.local_traj.duration * 2 / 3,
  ...
  for (double t = t_start; t < t_end; t += 0.03) {
    if ((traj_.local_traj.traj.getPos(t - my_traj_start_time) -
         traj_.swarm_traj[drone_id].traj.getPos(t - other_traj_start_time))
            .norm() <
        (getSwarmClearance() + traj_.swarm_traj[drone_id].des_clearance)) {
      return true;
    }
  }
...
bool EGOPlannerManager::planGlobalTrajWaypoints(
  ...
  for (int j = 0; j < 2; ++j) {
    ...
    if (globalMJO.getTraj().getMaxVelRate() < pp_.max_vel_ ||
        start_vel.norm() > pp_.max_vel_ || end_vel.norm() > pp_.max_vel_) {
      break;
    }

    if (j == 2) {
      fprintf(stderr, "[WARN] Global traj MaxVel = %f > set_max_vel\n",
              globalMJO.getTraj().getMaxVelRate());
      cout << "headState=" << endl << headState << endl;
      cout << "tailState=" << endl << tailState << endl;
    }

    des_vel /= 1.5;
  }

  traj_.setGlobalTraj(globalMJO.getTraj(), ego_now_s());

...
void loadVoxelConfig(
    ...
    VoxelMapConfig &voxel_config); // Stage 3: params instead of NodeHandle
...
public:
  ...
  // Stage 3: voxel_map_pub_ removed (no ROS); Stage 5 adds Foxglove
  // visualization
  ...
  Eigen::Quaterniond geoQuat_; // Stage 3: replaced geometry_msgs::Quaternion
  ...
  // Returns a cloud of confirmed voxel plane centroids (throttled to ~2 Hz),
  // or nullptr when there is nothing to publish this frame.
  // The caller (LIVMapper) pushes the result to bridge->voxel_map_queue.
  PointCloudXYZI::Ptr pubVoxelMap();
